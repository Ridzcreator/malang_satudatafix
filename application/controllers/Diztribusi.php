...
class Diztribusi extends CI_Controller {
    ...
    public function index() {
		$tahun="0000";
        $data['datasx']=$this->m_diztribusi->tampil_tahun();
    	  $data['data']=$this->m_diztribusi->tampil_jumlah($tahun);
        $data['bibit']=$this->m_diztribusi->tampil_bibit();
        ...
        $this->load->view('lingkunganhidup/v_distribusibibit', $data);
 ...
 //    }
      ...
      $this->m_diztribusi->simpan_barang($periode,$jenisbibit,$jumlah,$status,$penginput);
	...
	public function proses_edit_bibit(){
        ...
        $status = $this->input->post('status');
        $penginput = $this->input->post('penginput');
		$this->m_diztribusi->update_bibit($id,$periode,$jenisbibit,$jumlah,$status,$penginput);
		redirect('Diztribusi');	
	}	
	public function proses_delete_bibit(){
		$id=$this->input->post('kodeinput');
		$this->m_diztribusi->delete_bibit($id);
		redirect('Diztribusi');	
	}
   ...
   public function tampil_crosstab_bibit(){
      ...
      $data['data']=$this->m_diztribusi->tahun_crosstab();
      $data['tahun']=$this->m_diztribusi->tampil_periodec($tahun1,$tahun2);
      $data['jumlah']=$this->m_diztribusi->tampil_jumlahc($tahun1,$tahun2);
      $data['jenisbibit']=$this->m_diztribusi->tampil_bibitc();

        $this->load->view('template/header');
        $this->load->view('lingkunganhidup/tampil_crosstabdistribusi', $data);
        $this->load->view('template/footer');
     ...
     public function grafik_perbandinganx() {
      ...
      $data['data']=$this->m_diztribusi->grafik_Perbandingan_perumahanx($tahun1, $tahun2);
      $data['datax']=$this->m_diztribusi->grafik_Perbandingan_bibit($tahun1,$tahun2,$datap);
      $data['dataz']=$this->m_diztribusi->tahun_crosstab();
      $data['tahun']=$this->m_diztribusi->tampil_periodec($tahun1,$tahun2);
      $data['jumlah']=$this->m_diztribusi->tampil_jumlahc($tahun1,$tahun2);
      $data['jenisbibit']=$this->m_diztribusi->tampil_bibitc();
      // $data['dataz']=$this->m_diztribusi->tahun_crosstab();
      // $data['tahun']=$this->m_diztribusi->tampil_periodec($tahun1,$tahun2);
      // $data['jumlah']=$this->m_diztribusi->tampil_jumlahc($tahun1,$tahun2);
      // $data['jenis_bibit']=$this->m_diztribusi->tampil_bibita();

      $this->load->view('template/header');
      if($datap=="all"){
      if($grafikp=="bar"){
      $this->load->view('lingkunganhidup/grafik_distribusi_bar_all', $data);
      $this->load->view('template/footer');
      }else if($grafikp=="line"){
      $this->load->view('lingkunganhidup/grafik_distribusi_line_all', $data);
      $this->load->view('template/footer');
      }
    }
    }

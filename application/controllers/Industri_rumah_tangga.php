...
class Industri_rumah_tangga extends CI_Controller {
    ...
    public function index() {
        $periode=date('y');
        $tahun =0000;
    	$data['data']=$this->m_industri_kecil_rumah_tangga->tampil_industri_rumah_tangga($tahun);
        $data['datax']=$this->m_industri_kecil_rumah_tangga->tampil_tahun_rumah();
        $data['datas']=$this->m_industri_kecil_rumah_tangga->tampil_industri();
        $this->load->view('template/header');
        $this->load->view('perindustrian/industri_rumah_tangga',$data);
    ...
    public function tampil_grafik_industri_rumah_tangga(){
        ...
        $data['data']=$this->m_industri_kecil_rumah_tangga->tampil_industri_rumah_tangga($tahunx);
        $this->load->view('template/header');
        $this->load->view('perindustrian/grafik_industri_rumah_tangga',$data);
        $this->load->view('template/footer');
    }
      ...
      $data['datap']=$datap;
      $data['data']=$this->m_industri_kecil_rumah_tangga->grafik_perbandingan_industri_rumah_tanggax($tahun2, $tahun1);
      $this->load->view('template/header');
      if($datap=="all"){
        if($grafikp=="bar"){
        $this->load->view('perindustrian/grafik_perbandingan_industri_rumah_tangga_bar_all', $data);
        $this->load->view('template/footer');
        }else if($grafikp=="line"){
        $this->load->view('perindustrian/grafik_perbandingan_industri_rumah_tangga_line_all', $data);
        $this->load->view('template/footer');
        }
      }else{
      if($grafikp=="bar"){
        $this->load->view('perindustrian/grafik_perbandingan_industri_rumah_tangga_bar', $data);
        $this->load->view('template/footer');
        }else if($grafikp=="line"){
        $this->load->view('perindustrian/grafik_perbandingan_industri_rumah_tangga_line', $data);
        $this->load->view('template/footer');
      }
      }
     ...
     public function tampil_industri(){
        $this->load->view('template/header');
        $this->load->view('perindustrian/industri_rumah_tangga',$dataindustri);
        $this->load->view('template/footer');
    }


    public function proses_tambah_industri_rumah_tangga(){
        ...
        $cek = $this->db->query("SELECT * FROM industri_kecil_rumah_tangga where jenis_industri='$jenis' and tahun='$tahun' and status = 0");
           if ($cek->num_rows()>=1)
           {
            ...
            redirect('Industri_rumah_tangga','refresh');
           }
           else {
        ...
        redirect('Industri_rumah_tangga');

    }
}


    public function proses_hapus_industri_rumah_tangga(){
    ...
    public function proses_edit_industri_rumah_tangga(){
        $no=$this->input->post('no');
        $jenis=$this->input->post('jenis_industri');
        $j_unit=$this->input->post('jumlah_unit_produksi');
        $j_tenaga=$this->input->post('jumlah_tenaga_kerja');

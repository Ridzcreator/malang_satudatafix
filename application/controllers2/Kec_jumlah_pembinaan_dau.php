...
class Kec_jumlah_pembinaan_dau extends CI_Controller {
	...
	public function get() {
		...
		$tabel=array();
					...
					$temp = array();
						...
						$temp[]="<a class=\"btn btn-xs btn-success\" href=\"Kec_jumlah_pembinaan_dau/detail_kec_jumlah_pembinaan/$periode\" title=\"Detail\"><span class=\"fa fa-edit\"></span> Detail</a>| 
      ...
      foreach ($jenis as $key) {
       $cek = $this->db->query("SELECT * FROM kec_jumlah_pembinaan where kecamatan='".$kecamatan[$i]."' and jenis_pembinaan='".$jenis[$i]."' and periode='".$tahun[$i]."' and status = 0");
           if ($cek->num_rows()>=1)
           {
            ...
            redirect('Kec_jumlah_pembinaan_dau','refresh');
           }
    ...
    $test= $this->db->insert_batch('kec_jumlah_pembinaan', $data); // fungsi  untuk menyimpan multi array ke database
            ...
            redirect('Kec_jumlah_pembinaan_dau');  
           }
      }
	...
	public function proses_edit_kec_jumlah_pembinaan(){
		...
		if (!$test) {
            ...
            redirect('Kec_jumlah_pembinaan_dau','refresh');
					...
					redirect('Kec_jumlah_pembinaan_dau/detail_kec_jumlah_pembinaan/'.$page);	

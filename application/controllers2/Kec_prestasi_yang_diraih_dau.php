...
class Kec_prestasi_yang_diraih_dau extends CI_Controller {
	...
	public function get() {
		...
		$tabel=array();
					...
					$temp = array();
						...
						$temp[]="<a class=\"btn btn-xs btn-success\" href=\"Kec_prestasi_yang_diraih_dau/detail_kec_prestasi_yang_diraih/$periode\" title=\"Detail\"><span class=\"fa fa-edit\"></span> Detail</a>| 
      ...
      foreach ($jenis as $key) {
       $cek = $this->db->query("SELECT * FROM kec_penduduk_berdasarkan_prestasi where prestasi='".$jenis[$i]."' and kecamatan='".$kecamatan[$i]."' and periode='".$tahun[$i]."' and status = 0");
           if ($cek->num_rows()>=1)
           {
            ...
            redirect('Kec_prestasi_yang_diraih_dau','refresh');
           }
    ...
    $test= $this->db->insert_batch('kec_penduduk_berdasarkan_prestasi', $data); // fungsi  untuk menyimpan multi array ke database
            ...
            redirect('Kec_prestasi_yang_diraih_dau');  
           }
      }
	...
	public function proses_input_detailkec_banyak_lapangan_olahraga(){
		...
		redirect('Kec_banyak_lapangan_olahraga_dau/detailkec_banyak_lapangan_olahraga/'.$page);   	
	}	
	...
	public function proses_edit_kec_prestasi_yang_diraih(){
		...
		if (!$test) {
            ...
            redirect('Kec_prestasi_yang_diraih_dau','refresh');
					...
					redirect('Kec_prestasi_yang_diraih_dau/detail_kec_prestasi_yang_diraih/'.$page);	

<?php
class M_kelompokekonomi extends CI_Model {
...
var $tabel_kelurahan='master_desa';
	...
	public function simpan_barang($jumlah,$periode,$penginput,$status,$kecamatann,$desaa){
		...
		$hsl=$this->db->query("INSERT INTO kelompok_ekonomi(id_lembaga, kecamatan, desa, jumlah, periode, penginput, status) VALUES ('','$kecamatann','$desaa','$jumlah','$periode','$penginput','$status')");
		return $hsl;
	}
	...
	public function update_lembaga($id,$jumlah,$periode,$penginput,$status,$kecamatann,$desaa){
		$hsl=$this->db->query("UPDATE kelompok_ekonomi SET jumlah='$jumlah',periode='$periode', penginput='$penginput' WHERE id_lembaga='$id'");
		return $hsl;
	}
	public function delete_lembaga($id){
		$status=1;
		$hsl=$this->db->query("UPDATE kelompok_ekonomi SET status='$status' WHERE id_lembaga='$id'");
		return $hsl;
	...
	public function tampil_tahun(){
		$hsl=$this->db->query("SELECT periode FROM kelompok_ekonomi WHERE STATUS = '0' GROUP BY periode desc");
		return $hsl;
	}
	...
	public function tampil_jumlah($tahun){
		$thn=$tahun;
		if($thn=='0000'){
			$hsl=$this->db->query("select a.id_kecamatan, a.nama_kecamatan, b.id_desa,b.nama_desa,c.id_lembaga, c.kecamatan,c.desa,c.jumlah,c.periode from master_kecamatan as a join kelompok_ekonomi as c on a.id_kecamatan = c.kecamatan join master_desa as b on b.id_desa = c.desa where c.status=0 order by c.periode desc");
		}else{
			$hsl=$this->db->query("select a.id_kecamatan, a.nama_kecamatan, b.id_desa,b.nama_desa,c.id_lembaga, c.kecamatan,c.desa,c.jumlah,c.periode from master_kecamatan as a join kelompok_ekonomi as c on a.id_kecamatan = c.kecamatan join master_desa as b on b.id_desa = c.desa where c.status=0 and c.periode='$thn' ");
		}
	...
	public function tahun_crosstab(){
		$hsl=$this->db->query("select a.id_kecamatan, a.nama_kecamatan, b.id_desa,b.nama_desa,c.id_lembaga, c.kecamatan,c.desa,c.jumlah,c.periode from master_kecamatan as a join kelompok_ekonomi as c on a.id_kecamatan = c.kecamatan join master_desa as b on b.id_desa = c.desa where c.status=0  ORDER BY c.kecamatan");
		return $hsl;
	}
	public function tampil_kecamatanc(){
		$hsl=$this->db->query("select  a.nama_kecamatan from master_kecamatan as a join kelompok_ekonomi as c on a.id_kecamatan = c.kecamatan join master_desa as b on b.id_desa = c.desa where c.status=0 ORDER BY c.kecamatan");
		return $hsl;
	}
	public function tampil_desac(){
		$hsl=$this->db->query("select DISTINCT a.nama_kecamatan,b.nama_desa from master_kecamatan as a join kelompok_ekonomi as c on a.id_kecamatan = c.kecamatan join master_desa as b on b.id_desa = c.desa where c.status=0 ORDER BY c.kecamatan");
		return $hsl;
	}
	public function tampil_periodec($tahun1,$tahun2){
		$hsl=$this->db->query("select DISTINCT c.periode from master_kecamatan as a join kelompok_ekonomi as c on a.id_kecamatan = c.kecamatan join master_desa as b on b.id_desa = c.desa where c.status=0 and c.periode BETWEEN '$tahun1' and '$tahun2' order by c.periode");
		return $hsl;
	}
	...
	public function grafik_perbandingan_lembaga_desa($tahun){
		$hsl=$this->db->query("select a.id_kecamatan, a.nama_kecamatan, b.id_desa,b.nama_desa,c.id_lembaga, c.kecamatan,c.desa,c.jumlah,c.periode from master_kecamatan as a join kelompok_ekonomi as c on a.id_kecamatan = c.kecamatan join master_desa as b on b.id_desa = c.desa where c.status=0 and c.periode = '$tahun' ORDER BY c.kecamatan");
		return $hsl;
	}

...
class m_vaksinasi_avian extends CI_Model
{
	...
	public function tampil_vaksinasi($tahun){
		...
		if($thn=='0000'){
			$hasil=$this->db->query("SELECT id, kecamatan, desa, sum(jumlah) as jumlah, tahun FROM vaksinasi_avian WHERE status='0' GROUP BY tahun,kecamatan ORDER BY tahun ASC");
		}else{
			$hasil=$this->db->query("SELECT id, kecamatan, desa, sum(jumlah) as jumlah, tahun FROM vaksinasi_avian WHERE status='0' and tahun = '$thn' GROUP BY tahun,kecamatan");
		}
		return $hasil;
	}
	...
	public function tampil_detail_vaksinasi($page, $kcmtn){
		$hasil=$this->db->query("SELECT * FROM vaksinasi_avian inner join master_desa on vaksinasi_avian.desa = master_desa.id_desa WHERE status='0' AND tahun='$page' AND kecamatan='$kcmtn'");
		return $hasil;
	}
	...
	public function simpan_vaksinasi($kecamatan, $desa, $nama_ternak, $jumlah, $tahun, $penginput){
		$hasil=$this->db->query("INSERT INTO vaksinasi_avian
			(id, kecamatan, desa, nama_ternak, jumlah, tahun, penginput) 
			VALUES 
			('', '$kecamatan', '$desa', '$nama_ternak', '$jumlah', '$tahun', '$penginput')");
		return $hasil;
	}

	public function ubah_vaksinasi($id, $kecamatan, $desa, $nama_ternak, $jumlah, $tahun, $penginput){

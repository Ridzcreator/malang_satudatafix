...
class m_wisata_alam extends CI_Model
{
	...
	public function tampil_wisata_alam($kecamatan){
		$kcmtn=$kecamatan;
		if($kcmtn=='0000'){
			$hasil=$this->db->query("SELECT a.id,a.nama,a.latitude,a.longitude,a.fasilitas,a.pengelola,a.jenis_warisan,a.publikasi,a.aksesibilitas,a.tenaga_kerja,b.nama_kecamatan,c.nama_desa FROM wisata_alam AS a JOIN master_kecamatan AS b ON a.kecamatan = b.id_kecamatan JOIN master_desa AS c ON a.desa = c.id_desa WHERE STATUS = 0");
		}else{
	...
	public function simpan_wisata_alam($nama, $kecamatan, $desa, $latitude, $longitude, $fasilitas, $pengelola, $publikasi, $aksesibilitas, $tenaga_kerja, $penginput, $jenis_warisan){
		$hasil=$this->db->query("INSERT INTO wisata_alam
			(id, nama, kecamatan, desa, latitude, longitude, fasilitas, pengelola, publikasi, aksesibilitas, tenaga_kerja, penginput, jenis_warisan) 
			VALUES 
			('', '$nama', '$kecamatan', '$desa', '$latitude', '$longitude', '$fasilitas', '$pengelola', '$publikasi', '$aksesibilitas', '$tenaga_kerja', '$penginput', '$jenis_warisan')");
		return $hasil;
	}

	function ubah_wisata_alam($id, $nama, $desa, $latitude, $longitude, $fasilitas, $pengelola, $publikasi, $aksesibilitas, $tenaga_kerja, $penginput, $jenis_warisan){
		$hasil=$this->db->query("UPDATE wisata_alam 
			SET nama='$nama', kecamatan='$kecamatan', desa='$desa', latitude='$latitude', logitude='$logitude', fasilitas='$fasilitas', pengelola='$pengelola', publikasi='$publikasi', aksesibilitas='$aksesibilitas', tenaga_kerja='$tenaga_kerja', penginput='$penginput', jenis_warisan='$jenis_warisan' where id='$id'");
		return $hasil;
	}

...
class m_wisata_budaya extends CI_Model
{
	...
	public function tampil_wisata_budaya($kecamatan){
		$kcmtn=$kecamatan;
		if($kcmtn=='0000'){
			$hasil=$this->db->query("SELECT a.id,a.nama,a.fasilitas,a.deskripsi,a.pengelola,a.jenis_warisan,a.publikasi,a.aksesibilitas,a.latitude,a.longitude, b.nama_kecamatan,c.nama_desa FROM wisata_budaya AS a JOIN master_kecamatan AS b ON a.kecamatan = b.id_kecamatan JOIN master_desa AS c ON a.desa = c.id_desa WHERE a.STATUS = 0 ");
		}else{
	...
	public function simpan_wibu($nama, $kecamatan, $desa, $latitude, $longitude, $fasilitas, $deskripsi, $pengelola, $publikasi, $aksesibilitas, $penginput, $jenis_warisan){
		$hasil=$this->db->query("INSERT INTO wisata_budaya
			(id, nama, kecamatan, desa, latitude, longitude, fasilitas, deskripsi, pengelola, publikasi, aksesibilitas, penginput, jenis_warisan) 
			VALUES 
			('', '$nama', '$kecamatan', '$desa', '$latitude', '$longitude', '$fasilitas', '$deskripsi', '$pengelola', '$publikasi', '$aksesibilitas', '$penginput', '$jenis_warisan')");
		return $hasil;
	}

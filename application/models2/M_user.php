...
class M_user extends CI_Model {
	...
	public function simpan_barang($user,$password,$level,$penginput){
		...
		$hsl2=$this->db->query("select * from user where username='$user' and password='$password1' and where status='0'");
		$count = $hsl2->num_rows();
		if($count == 0)
		{	
	...
	public function tampil_user(){
		$hsl=$this->db->query("SELECT user.id, user.username, user.password, level.keterangan, user.level, user.status from user as user inner join master_level as level on user.level=level.kode where user.status = '0'");
		return $hsl;
	}

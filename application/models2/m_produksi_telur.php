...
class m_produksi_telur extends CI_Model
{
	...
	public function tampil_produksi_telur($tahun){
		$thn=$tahun;
		if($thn=='0000'){
			$hasil=$this->db->query("SELECT id, tahun, kecamatan, sum(per_kg) as per_kg, sum(per_butir) as per_butir FROM produksi_telur WHERE status='0' GROUP BY tahun,kecamatan ORDER BY tahun ASC");
		}else{
			$hasil=$this->db->query("SELECT id, tahun, kecamatan, sum(per_kg) as per_kg, sum(per_butir) as per_butir FROM produksi_telur WHERE status='0' and tahun='$thn' GROUP BY tahun,kecamatan ");
		}
		return $hasil;
	}
	...
	public function simpan_produksi_telur($kecamatan, $nama_unggas, $tahun, $per_butir, $per_kg, $penginput){
		$jumlah=0;
		$jumlah=$per_butir+$per_kg;
		$hasil=$this->db->query("INSERT INTO produksi_telur
			(id, kecamatan, hewan, tahun, per_butir, per_kg, jumlah, penginput) 
			VALUES
			('', '$kecamatan', '$nama_unggas', '$tahun', '$per_butir', '$per_kg', '$jumlah', '$penginput')");
		return $hasil;
	}

...
<div class="content-wrapper">
    ...
    <div class="box">
        <div class="box-header">
            <h3 class="box-title">Laporan Crosstab Produksi Daging</h3>
        </div>
        <div class="box-body">
        <table>
        <tr>
        ...
        foreach ($tahun->result_array() as $row1) {
         ...
         $array_daging=array();
        foreach ($jenis_daging->result_array() as $row2) {
           
            array_push($array_daging, $row2['jenis_daging']);
        }
        ...
        array_push($array_data_1, $row4['jenis_daging']);
        ...
        $temp = 0;  
        for ($i=0; $i < count($array_daging) ; $i++) { 
            for ($j=0; $j < (count($array_tahun)) ; $j++) { 
                $isinya = '';
                for ($x=0; $x < count($array_data_1) ; $x++) { 
                    if ($array_daging[$i]==$array_data_1[$x]&& $array_tahun[$j]==$array_data_3[$x]) {
        ...
        <table class="table table-bordered table-striped" style="font-size:15px; width:100%; text-align:center; " id="tampilDetailCrosstabTelur">
                <thead>
                   <tr>
                 <th rowspan="2" style="text-align: center; vertical-align: middle;">Jenis Daging</th>
                 <?php 
